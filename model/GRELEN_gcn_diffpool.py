...
# encoders.py与GRELEN_gcn_diffpool.py位于同级子目录下
...
class grelen_diffpool(torch.nn.Module):
    def __init__(self, device, num_units, max_diffusion_step, num_nodes, nonlinearity='tanh',
        ...
        # 替换为SoftPoolingGcnEncoder
        self.gcn_encoder = SoftPoolingGcnEncoder(
            max_num_nodes=num_nodes, 
            input_dim=2 * num_units, 
            hidden_dim=num_units, 
            embedding_dim=num_units,
            label_dim=2 * num_units,  # 需要确保最终输出是2*num_units
            num_layers=2, 
    ...
    def forward(self, inputs, hx, adj):
        """
        inputs: 输入特征 [batch_size, num_nodes, input_dim]
        hx: 隐藏状态 [batch_size, num_nodes * num_units]
        adj: 邻接矩阵 [batch_size, num_nodes, num_nodes]
        """
        batch_size = inputs.shape[0]
        
        # Reshape hx to match the input dimensionality expected by the GCN encoder
        hx = hx.view(batch_size, self._num_nodes, -1)  # [batch_size, num_nodes, num_units]
        
        # 将inputs与hx拼接作为新的输入
        inputs_and_state = torch.cat([inputs, hx], dim=-1)  # [batch_size, num_nodes, 2 * num_units]
        
        # 使用SoftPoolingGcnEncoder进行前向传播
        ypred = self.gcn_encoder(inputs_and_state, adj)  # 输出形状为 [batch_size, 2 * num_units]
        
        # 将预测结果 reshape 为 [batch_size, num_nodes * output_size]
        new_state = ypred.view(batch_size, -1)  # [batch_size, num_nodes * num_units * 2]
...
class EncoderModel(nn.Module):
    def __init__(self, device, n_dim, n_hid, max_diffusion_step, num_nodes, num_rnn_layers, filter_type):
        ...
        # 创建了num_rnn_layers个单元,每层用于处理输入数据和邻接矩阵,并逐层递归更新隐藏状态
        self.dcgru_layers = nn.ModuleList(
            [grelen_diffpool(self.device, self.rnn_units, self.max_diffusion_step, self.num_nodes,
                       filter_type=self.filter_type) for _ in range(self.num_rnn_layers)])  # 定义多个 grelen_diffpool 单元
