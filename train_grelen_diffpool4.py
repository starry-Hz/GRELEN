...
#####################################################################修改模型
from model.GRELEN_diffpool_4 import *
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename='./log/train_grelen_diffpool3.log',
                    filemode='a')
...
def val_epoch(net, val_loader, sw, epoch, config):
    ...
    with torch.no_grad():   # 验证过程中不需要反向传播
        ...
        # print('epoch: %s, diffpool loss: %.2f' % (epoch, validation_loss))
        # logging.info('epoch: %s, diffpool loss: %.2f' % (epoch, validation_loss))
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, 300):
        ...
        diffpool_train_ = torch.tensor(diffpool_train)

        # print(f"epoch:{epoch},kl loss:{kl_train_.mean()},nll loss:{nll_train_.mean()},diffpool loss:{diffpool_train_.mean()}")
        # logging.info(f"epoch:{epoch},kl loss:{kl_train_.mean()},nll loss:{nll_train_.mean()},diffpool loss:{diffpool_train_.mean()}")
        
        print('epoch: %s, diffpool loss: %.2f' % (epoch, diffpool_train_.mean()))
        logging.info('epoch: %s, diffpool loss: %.2f' % (epoch, diffpool_train_.mean()))
        ...
        total_time += elapsed
        print(f"epoch:{epoch},elapsed time:{elapsed},total time:{total_time}")
        logging.info(f"epoch:{epoch},elapsed time:{elapsed},total time:{total_time}")
...
# logging.basicConfig(level=logging.DEBUG,
#                     format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
#                     filename='./log/train_grelen_diffpool_4.log',
#                     filemode='a')
